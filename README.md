...
float CorrectPerspectiveDepth(float rawDepth, float2 uv)
{
    float2 centeredUV = uv - 0.5;
    float aspect = _ScreenParams.x / _ScreenParams.y;
    centeredUV.x *= aspect;
    ...
    float normalizedDepth = rawDepth / (far - near);
    float correctedDepth = clamp(normalizedDepth * depthScale * (far - near), near, far);
